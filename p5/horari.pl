...
studentCourse(S, C) :- student(S, CS), member(C, CS).
courseRoom(C, R) :- courseRooms(C, CR), member(R, CR).
...
writeClauses(MaxLecturesLate) :-
    ...
    atMostOneHourPerDateAndCourse, % Done
    atMostOneCoursePerDateAndHourForAStudent, % Done
    atMostOneCoursePerRoomAndDayAndHour,
    atMostMaxLecturesLate(MaxLecturesLate),
    true.


defineLate :-
    day(D),
    findall(courseHour-C-D-5, course(C), Lits),
    expressOr(late-D, Lits),
    fail.
...
exactlyOneRoomPerCourse :-
    course(C),
    findall(courseRoom-C-R, courseRoom(C,R), Lits),
    exactly(1, Lits),
    fail.
...
atMostOneCoursePerDateAndHourForAStudent :-
    student(S), hour(H), day(D),
    findall(courseHour-C-D-H, studentCourse(S, C), Lits),
    atMost(1, Lits),
    fail.
...
atMostOneCoursePerRoomAndDayAndHour :-
    ...
    writeClause([\+courseRoom-C1-R, \+courseRoom-C2-R, \+courseHour-C1-D-H, \+courseHour-C2-D-H]),
    fail.
...
tryWith(MaxLecturesLate, _) :-
    ...
    write('Generated '), write(C), write(' clauses over '), write(N), write(' variables. '),nl,
	  ...
	  shell('picosat -v -o model infile.cnf', 10),  % if sat: Result=10; if unsat: Result=20.
    see(model), symbolicModel(M), seen,
    countLecturesLate(M, LL),
    write('Solution found with '), write(LL), write(' lectures late!'), nl,
...
countLecturesLate(_, X) :- lecturesLateCount(X).
