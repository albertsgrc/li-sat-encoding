...
% Potser restringir que els alumnes tinguin TOTES LES SEVES CLASES?
...
atMostOneCoursePerRoomAndDayAndHour :-
    room(R), day(D), hour(H),
    findall(courseHour-C-D-H, courseRoom(C, R), Lits),
    atMost(1, Lits),
    false.
...
tryWith(MaxLecturesLate, PreviousSolution) :-
  initClauseGeneration,
  tell(clauses), writeClauses(n), told, % generate the (numeric) SAT clauses and call the solver
