...
defineHome:-
  ...
  expressOr( home-T-R, Lits ),
  fail.
defineHome.

defineDouble:-
  team(T), roundGreaterOne(R), Ra is R-1,
  expressEq( double-T-R, [home-T-R, home-T-Ra] ),
  fail.
defineDouble.

atMostOneTvTeamMatchEachRound :-
  round(R),
  findall( match-T-S-R, tvMatch(T-S), Lits ),
  atMost(1, Lits),
  fail.
atMostOneTvTeamMatchEachRound.
...
exactlyOneMatchPerTeamEachRound :-
  round(R), team(T),
  findall( match-X-Y-R, matchOfT(T, X-Y), Lits ),
  exactly(1, Lits),
  fail.
exactlyOneMatchPerTeamEachRound.

exactlyOneMatchPerPairOfTeams :-
  team(T), matchOfT(T, S-T),
  findall( match-X-Y-R, ( member([X,Y], [[S,T],[T,S]]), round(R) ), Lits ),
  exactly(1, Lits),
  fail.
exactlyOneMatchPerPairOfTeams.

noDoublesInCertainRounds :-
  noDoubles(RND), member(R, RND), team(T),
  negate(double-T-R, NDRT),
  writeClause([NDRT]),
  fail.
noDoublesInCertainRounds.
