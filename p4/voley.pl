...
roundGreaterOne(R) :- between(2,15,R).
...
writeClauses:-
    defineHome,
    defineDouble,
    atMostOneTvTeamMatchEachRound,
    exactlyOneMatchPerTeamEachRound,
    exactlyOneMatchPerPairOfTeams,
    noDoublesInCertainRounds,
    atMostOneDoubleForTeam. % atMostOne(match-x-y-r) Vr ronda, Vx,y in tvTeams,

defineHome:- team(T), round(R), findall( match-T-S-R, matchOfT(T,T-S), Lits ), expressOr( home-T-R, Lits ), fail.
defineHome. % Correcte

defineDouble:- team(T), roundGreaterOne(R), Ra is R-1,  expressEq(double-T-R, [home-T-R, home-T-Ra]), fail.
defineDouble. % Correcte

atMostOneTvTeamMatchEachRound :- round(R), findall( match-T-S-R, tvMatch(T-S), Lits ), atMost(1, Lits), fail.
atMostOneTvTeamMatchEachRound. % Correcte

% Optimitzable a exactly(1, [home-S-R, !home-S-R]) pertot S, R
exactlyOneMatchPerTeamEachRound :- round(R), team(T), findall( match-T-S-R, matchOfT(T, T-S), Lits ), exactly(1, Lits), fail.
exactlyOneMatchPerTeamEachRound.

exactlyOneMatchPerPairOfTeams :- team(T), matchOfT(T, S-T), findall( match-T-S-R, round(R), Lits), exactly(1, Lits), fail.
exactlyOneMatchPerPairOfTeams.

noDoublesInCertainRounds :- noDoubles(ND), member(Rnd, ND), team(T), negate(double-T-Rnd, Ndtr), writeClause([Ndtr]), fail.
noDoublesInCertainRounds.

% els atMost, exactly, etc han de rebre llistes!
atMostOneDoubleForTeam :- team(T), findall( double-T-R, round(R), Lits), atMost(1, Lits), fail.
atMostOneDoubleForTeam.
...
% Express that Var is equivalent to the equality X=Y of the pair [X,Y]
expressEq( V, [X,Y] ) :-
    negate(V, NV), negate(X, NX), negate(Y, NY),
    writeClause([NV, NX, Y]), writeClause([NV, X, NY]),
    writeClause([V, NX, NY]), writeClause([V, X,  Y]).
